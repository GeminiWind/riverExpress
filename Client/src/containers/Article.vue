      ...
      <div class="col-lg-8 col-md-8">
        <div class="content_bottom_left">
          <div class="single_page_area">
            <ol class="breadcrumb">
              ...
              <li><router-link :to="{ name: 'category', params: { category: article.category }}">{{ article.category }}</router-link></li>
              <li class="active">{{ article.title}} </li>
            </ol>
            <h2 class="post_titile">{{ article.title }}</h2>
            <div class="single_page_content">
              <div class="post_commentbox"> <a href="#"><i class="fa fa-user" v-if="article.author.name"></i>{{ article.author.name }}</a> <span><i class="fa fa-calendar"></i>{{ article.created_at | moment("dddd, MMMM Do YYYY, h:mm a") }}</span> <a href="#"><i class="fa fa-tags"></i>Technology</a> </div>
       ...
       <div v-if="user">
        <div class="comment_encourage" v-if="article.comments.length === 0">
          ...
          <div class="pull-left"> You are commenting as <vue-letter-avatar name=user.profile.first_name size='25'></vue-letter-avatar></div>
          <button class="btn btn-primary pull-right" @click="sendComment">Send</button>
          <hr/>
          <h3>Comments</h3>
          <div class="comment_area" v-for="comment in article.comments" v-if="article.comments.length > 0">
            <div class="row">
              <div class="col-md-3">
        ...
        <div class="similar_post">
          ...
          <ul class="small_catg similar_nav wow fadeInDown animated">
            <li>
              <div class="media wow fadeInDown animated" style="visibility: visible; animation-name: fadeInDown;"> <a class="media-left related-img" href="#"><img src="../assets/images/112x112.jpg" alt=""></a>
...
export default {
  ...
  methods: {
    async fetch(articleSlug){
      try {
        const { data } = await this.$http.get(`/articles/${articleSlug}`);
    ...
    async sendComment() {
      try {
        ...
        const { data } = await this.$http.post(`/articles/${articleSlug}/comment`, { content: this.comment});
